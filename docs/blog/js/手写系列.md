...
```
## isArray
> 判断一个变量是否是数组,es6中有 Array.isArray 方法。以下是替代方法
...
```js
function flat() {
	let new_arr = this.reduce((acc, value) => {
		acc.concat(Array.isArray(value) ? flat(value) : value);
	}, []);
}
```
...
> JSON.stringify 方法将对象转换为 json 字符串，由于 undefined ，函数 ，Symbol 格式的数据在 JSON.stringify 中会被置为 `null（数组）`或者 `忽略（对象属性值中出现，key被忽略）`，循环引用会直接报错。正常数字都可以转换为字符串，NaN和Infinity 在数组和对象中都会转化为null
...
function jsonStringify(obj) {
	...
	if (type !== "object" || type === null) {
		...
		if (arr) {
			...
			res = [];
			// 如果是对象，则对每一项做递归, 并作为value 存入 数组中
			for (key in obj) {
...
var time = new Date()
var times = time.format('yyyy-MM-dd hh:mm:ss S')
console.log(time)
console.log(times)
```