...
> JSON.stringify 方法将对象转换为 json 字符串，由于 undefined ，函数 ，Symbol 格式的数据在 JSON.stringify 中会被置为 `null（数组）`或者 `忽略（对象属性值中出现，key被忽略）`，循环引用会直接报错

```js
// 主要在于处理 undefined Symbol 和 function 函数的字符串转换
