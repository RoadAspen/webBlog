...
`
 ...
 The Performance Budget
    ...
    Phase  Time  What Happens      Fast-path check  ~5ms  Check argv, exit early if possible    Module evaluation  ~138ms  Import tree, fire parallel I/O    Commander parse  ~3ms  Parse flags and subcommands    init()  ~14ms  Config resolution, trust boundary    setup()  ~35ms  Commands, agents, hooks, plugins    Launch + first render  ~25ms  Pick path, mount React, first paint     Total    ~240ms   Under 300ms budget
